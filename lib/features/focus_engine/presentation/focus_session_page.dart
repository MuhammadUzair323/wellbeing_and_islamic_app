...
class _FocusSessionPageState extends State<FocusSessionPage>
  ...
  void _onTick(Duration elapsed) {
    ...
    // Calculate how many whole seconds we have accumulated
    final wholeSecondsAccumulated = _fractionalSecondsAccumulated.floor();

    // If we've accumulated at least one whole second, update the remaining time
    if (wholeSecondsAccumulated > 0) {
      setState(() {
        _remainingSeconds = (_remainingSeconds - wholeSecondsAccumulated)
            .clamp(0, _totalSeconds);
        // Remove the whole seconds we've accounted for from our accumulator
        _fractionalSecondsAccumulated -= wholeSecondsAccumulated;
